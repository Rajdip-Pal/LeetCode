...
{
    static void
    prepare_level_order_sum(const TreeNode *root, std::queue<const TreeNode *> &nodeQueue, std::unordered_map<int, size_t> &result)
    {
        if (root)
        {
            int level{}, levelNodeCount{};
            size_t levelSum{};
            nodeQueue.push(root);
            ...
            while (levelNodeCount)
            {
                ...
                level++, levelSum = 0, levelNodeCount = nodeQueue.size();
            }
        }
    }

    static void update_tree_from_parent(TreeNode *root, const int &level, const std::unordered_map<int, size_t> &levelOrderSumMap)
    {
        if (root->left and root->right)
        {
            size_t value =
                levelOrderSumMap.at(level) - root->left->val - root->right->val;
    ...
    static void
    update_tree(TreeNode *root, const int &level, const std::unordered_map<int, size_t> &levelOrderSumMap)
    {
        if (root)
        {
            root->val = 0;
...
public:
    static TreeNode *replaceValueInTree(TreeNode *root)
    {
        ...
        std::unordered_map<int, size_t> levelOrderSumMap;
