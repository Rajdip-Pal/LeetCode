...
std::vector<int> left_max_height(std::vector<int>& example){
    std::vector<int> temp;
    for (int i = 0; i < example.size(); i++){
        if (i == 0 || example[i] > temp[i-1])
            temp.push_back(example[i]);
        else
...
std::vector<int> right_max_height(std::vector<int>& example){
    std::vector<int> temp(example.size());
    for (int i = example.size() - 1; i >= 0; i--){
        if (i == example.size() - 1 || example[i] > temp[i+1])
            temp[i] = example[i];
        else
            temp[i] = temp[i+1];
    }
...
int trapped_water(std::vector<int>& height){
    std::vector<int> left_max = left_max_height(height);
    std::vector<int> right_max = right_max_height(height);
    int trapped_water = 0;
...
int total_trapped_water(std::vector<std::vector<int>>& heightMap){
    int row = heightMap.size();
    int row_trapped_water[row];
    for (int i = 0; i < row; i++){
        row_trapped_water[i] = trapped_water(heightMap[i]);
    }
    int column = heightMap[0].size();
    int column_trapped_water[column];
    for (int i = 0; i < column; i++){
        ...
        temp_vector.assign(temp,temp+row);
        delete[] temp;
        column_trapped_water[i] = trapped_water(temp_vector);
    }
    int result1 {}, result2 {};
    for (int i = 0; i < row; i++)
        result1 += row_trapped_water[i];
    for (int j = 0; j < column; j++)
        result2 += column_trapped_water[j];
...
int main() {
    ...
    std::cout << total_trapped_water(heightMap);
