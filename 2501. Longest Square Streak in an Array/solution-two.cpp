...
public:
    static int longestSquareStreak(const std::vector<int> &nums)
    {
        ...
        int maxStreak{-1};
        std::unordered_set<unsigned long> squareCollections, givenSquaredElements;

        {
            unsigned long square{};
            for (unsigned long i = 2; (square = i * i) <= 100000; i++)
                squareCollections.insert(square);
        }

        {
            for (int i{}; i < nums.size(); i++)
                if (squareCollections.find(nums[i]) != squareCollections.end())
                    givenSquaredElements.insert(nums[i]);
        }

        {
            ...
            {
                if (givenSquaredElements.find(currentNum = powl(nums[i], 2)) != givenSquaredElements.end())
                {
                    currentStreak += 2;
                    while (givenSquaredElements.find(currentNum *= currentNum) != givenSquaredElements.end())
                        currentStreak++;
